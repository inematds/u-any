...
> 🇬🇧 Versão original em inglês: [2026-03-14-phase1-implementation.md](./2026-03-14-phase1-implementation.md)

> **Para o Claude:** SUB-SKILL OBRIGATÓRIA: Use superpowers:executing-plans para implementar este plano tarefa a tarefa.

**Objetivo:** Construir o MVP fundacional — um monorepo pnpm com um motor de análise central (LLM + tree-sitter), um comando de skill `/understand` e um dashboard React básico com visualização em grafo e visualizador de código.

**Arquitetura:** Monorepo com 3 pacotes (core, skill, dashboard) compartilhando um schema JSON de grafo de conhecimento. O pacote core lida com análise e persistência. A skill invoca o core e abre o dashboard. O dashboard lê o JSON e renderiza um workspace multi-painel.

**Stack Técnica:** TypeScript, pnpm workspaces, Vitest, React 18, Vite, @xyflow/react (React Flow v12), @monaco-editor/react, Zustand, TailwindCSS, tree-sitter

---

## Tarefa 1: Estruturação do Projeto — Raiz do Monorepo
...
**Passo 2: Criar pnpm-workspace.yaml**
...
**Passo 3: Criar tsconfig.json raiz**
...
## Tarefa 2: Pacote Core — Estruturação e Tipos do Grafo de Conhecimento

**Arquivos:**
- Criar: `packages/core/package.json`
- Criar: `packages/core/tsconfig.json`
- Criar: `packages/core/src/index.ts`
- Criar: `packages/core/src/types.ts`

**Passo 1: Criar packages/core/package.json**
...
**Passo 2: Criar packages/core/tsconfig.json**
...
**Passo 3: Criar packages/core/src/types.ts**

Este é o sistema completo de tipos do Grafo de Conhecimento, vindo do documento de design:
...
**Passo 4: Criar packages/core/src/index.ts**

```typescript
export * from "./types.js";
```

**Passo 5: Executar pnpm install e build**
...
**Passo 6: Escrever um teste de validação de tipos**

Criar: `packages/core/src/types.test.ts`
...
## Tarefa 3: Pacote Core — Persistência em JSON

**Arquivos:**
- Criar: `packages/core/src/persistence/index.ts`
- Criar: `packages/core/src/persistence/persistence.test.ts`

**Passo 1: Escrever o teste que falha**

Criar: `packages/core/src/persistence/persistence.test.ts`

(Ver código original — preservado integralmente abaixo.)
...
**Passo 3: Implementar o módulo de persistência**
...
**Passo 4: Atualizar packages/core/src/index.ts**
...
Execute: `pnpm --filter @understand-anything/core test`
Esperado: Todos os 6 testes de persistência PASS + 3 testes de tipos PASS = 9 no total
...
**Arquivos:**
- Criar: `packages/core/src/plugins/tree-sitter-plugin.ts`
- Criar: `packages/core/src/plugins/tree-sitter-plugin.test.ts`

**Passo 1: Instalar dependências do tree-sitter**
...
**Passo 3: Executar teste para verificar que falha**

Execute: `pnpm --filter @understand-anything/core test`
Esperado: FAIL — módulo não encontrado

**Passo 4: Implementar o plugin tree-sitter**
...
**Passo 5: Atualizar packages/core/src/index.ts**
...
Execute: `pnpm --filter @understand-anything/core test`
Esperado: Todos os testes do tree-sitter PASS. Alguns testes podem precisar de ajuste com base na saída exata do parse do tree-sitter — itere até ficar verde.
...
**Arquivos:**
- Criar: `packages/core/src/analyzer/llm-analyzer.ts`
- Criar: `packages/core/src/analyzer/llm-analyzer.test.ts`
- Criar: `packages/core/src/analyzer/graph-builder.ts`
- Criar: `packages/core/src/analyzer/graph-builder.test.ts`

**Passo 1: Escrever o teste do graph builder**

O graph builder recebe a análise estrutural + sumários de LLM e monta um KnowledgeGraph.
...
**Passo 3: Implementar o GraphBuilder**
...
**Passo 5: Criar a interface do analisador LLM**

Criar: `packages/core/src/analyzer/llm-analyzer.ts`

Define a interface para análise baseada em LLM. As chamadas reais ao LLM acontecem via skill (que tem acesso à sessão do Claude). O pacote core define os prompts e o formato de resposta esperado.
...
**Passo 6: Escrever testes para o analisador LLM**
...
**Passo 7: Atualizar packages/core/src/index.ts**
...
## Tarefa 6: Pacote Dashboard — Estruturação com Vite + React

**Arquivos:**
- Criar: `packages/dashboard/` (via scaffold do Vite e depois customizar)

**Passo 1: Estruturar app React com Vite**

Execute: `cd packages && pnpm create vite dashboard --template react-ts`
Depois: Remova o boilerplate (App.css, etc.), mantenha a estrutura.

**Passo 2: Instalar dependências do dashboard**
...
**Passo 4: Adicionar dependência de workspace no core**

Adicione a `packages/dashboard/package.json` em dependencies:
...
**Passo 5: Criar a store Zustand**

Criar: `packages/dashboard/src/store.ts`
...
## Tarefa 7: Dashboard — Painel de Visualização em Grafo com React Flow

**Arquivos:**
- Criar: `packages/dashboard/src/components/GraphView.tsx`
- Criar: `packages/dashboard/src/components/CustomNode.tsx`

**Passo 1: Criar o componente de node customizado**
...
**Passo 2: Criar o componente GraphView**
...
**Arquivos:**
- Criar: `packages/dashboard/src/components/CodeViewer.tsx`

**Passo 1: Criar o componente CodeViewer**
...
**Arquivos:**
- Criar: `packages/dashboard/src/components/SearchBar.tsx`
- Criar: `packages/dashboard/src/components/NodeInfo.tsx`
- Modificar: `packages/dashboard/src/App.tsx`

**Passo 1: Criar o componente SearchBar**
...
**Passo 2: Criar o painel NodeInfo (placeholder para painéis de chat + learn)**
...
**Passo 3: Criar o layout principal do App**

Substituir: `packages/dashboard/src/App.tsx`
...
Esperado: Servidor de desenvolvimento Vite inicia em localhost:5173. O dashboard renderiza com a mensagem "No knowledge graph loaded".

**Passo 5: Testar com dados de amostra**

Crie `packages/dashboard/public/knowledge-graph.json` com um grafo de amostra para teste:
...
Esperado: Dashboard carrega, o grafo renderiza 5 nodes com edges, clicar em um node mostra detalhes no painel de info e um placeholder no visualizador de código. A busca funciona.
...
Após concluir todas as 10 tarefas:

1. **`pnpm install`** — Sem erros
2. **`pnpm --filter @understand-anything/core build`** — Compila limpo
3. **`pnpm --filter @understand-anything/core test`** — Todos os testes passam (types, persistence, tree-sitter, graph builder, LLM analyzer)
4. **`pnpm dev:dashboard`** — Dashboard inicia em localhost:5173
5. **Dashboard com dados de amostra** — Carrega `knowledge-graph.json`, grafo renderiza, nodes clicáveis, busca funciona, visualizador de código mostra info do node
