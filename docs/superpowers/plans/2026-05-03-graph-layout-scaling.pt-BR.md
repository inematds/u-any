# Plano de Implementação de Escalabilidade do Layout de Grafo

> 🇬🇧 Versão original em inglês: [2026-05-03-graph-layout-scaling.md](./2026-05-03-graph-layout-scaling.md)
...
**Objetivo:** Substituir dagre por ELK em todas as views de dashboard em estilo estrutural, adicionar containers baseados em pasta/comunidade na view de detalhe de camada, e computar o layout em dois estágios lazy para que camadas com muitos nodes sejam legíveis e grafos grandes permaneçam performáticos.

**Arquitetura:** Três views (overview, DomainGraphView, layer-detail) chamam um novo `applyElkLayout` em vez de `applyDagreLayout`. A view layer-detail ganha um passo `deriveContainers` (estratégia de pasta com fallback Louvain), edges entre containers agregados, chamadas lazy de ELK em dois estágios (Estágio 1 = containers; Estágio 2 = os filhos de um container sob demanda), um novo tipo de node React Flow `ContainerNode` e extensões de store para estado de expand e caches de layout.

**Stack:** TypeScript, React 19, Vite, React Flow (`@xyflow/react`), Zustand, Vitest, ELK.js (`elkjs`), `graphology` + `graphology-communities-louvain`.
...
- [ ] **Passo 1: Add deps and devDeps to package.json**
...
- [ ] **Passo 2: Update vite.config.ts to register vitest**
...
Esperado: pnpm resolves and installs without errors.

- [ ] **Passo 4: Write smoke test**
...
## Tarefa 2: deriveContainers — estratégia de pasta + casos de borda
...
- [ ] **Passo 2: Run tests to verify they fail**

```bash
pnpm --filter @understand-anything/dashboard test containers
```

Esperado: import error — `Cannot find module '../containers'`.

- [ ] **Passo 3: Implement `containers.ts`**
...
- [ ] **Passo 4: Stub `louvain.ts` (real impl in Task 3)**
...
## Tarefa 3: deriveContainers — fallback de comunidade (Louvain)
...
- [ ] **Passo 1: Add failing test for community fallback**
...
- [ ] **Passo 2: Run tests, expect failure**

```bash
pnpm --filter @understand-anything/dashboard test containers
```

Esperado: the new tests fail because the louvain stub puts every node in community 0 (only 1 container after suppression).

- [ ] **Passo 3: Replace louvain stub with real implementation**
...
Esperado: import error — `aggregateContainerEdges` not exported.
...
## Tarefa 5: Reparo de input do ELK
...
- [ ] **Passo 1: Write failing tests for repair functions**
...
Esperado: import error — `../elk-layout` not found.

- [ ] **Passo 3: Implement repairElkInput**
...
- [ ] **Passo 2: Add fields to interface**
...
- [ ] **Passo 3: Add initializer + methods**
...
- [ ] **Passo 4: Hook clearing into existing graph-load action**
...
- [ ] **Passo 5: Sanity-check by building**
...
- [ ] **Passo 2: Build to verify it type-checks**
...
- [ ] **Passo 3: Add helpers `clusterNodesToElkInput` and `mergeElkPositions`**
...
- [ ] **Passo 4: Manually verify overview**
...
- [ ] **Passo 1: Read current DomainGraphView**

Open the file and find every `applyDagreLayout(...)` call.

- [ ] **Passo 2: Replace with ELK in the same async pattern**
...
## Tarefa 11: Layer-detail Estágio 1
...
- [ ] **Passo 1: Register the ContainerNode type**
...
- [ ] **Passo 2: Inside `useLayerDetailTopology`, add container derivation**
...
- [ ] **Passo 3: Build Stage 1 ELK input from containers + ungrouped**
...
- [ ] **Passo 4: Replace `useMemo` with `useState + useEffect`**
...
## Tarefa 12: Layer-detail Estágio 2 + expansão de edge

**Arquivos:**
- Modificar: `understand-anything-plugin/packages/dashboard/src/components/GraphView.tsx`

- [ ] **Passo 1: Add Stage 2 effect**
...
- [ ] **Passo 2: Render expanded children + replace edges in visual overlay**
...
- [ ] **Passo 1: Zoom-driven auto-expand inside the ReactFlow component**
...
- [ ] **Passo 2: Search/focus/tour auto-expand**
...
- [ ] **Passo 1: Compute container overlay data in `useLayerDetailGraph`**
...
## Tarefa 15: Re-layout do Estágio 2 por desvio de tamanho

**Arquivos:**
- Modificar: `understand-anything-plugin/packages/dashboard/src/components/GraphView.tsx`

- [ ] **Passo 1: Detect deviation after Stage 2**
...
## Tarefa 16: Copy do WarningBanner para issues de layout + overlay "Computing layout"
...
- [ ] **Passo 1: Add layout-issue funnel to store**
...
- [ ] **Passo 2: Funnel ELK issues into store after each layout call**
...
- [ ] **Passo 3: Display banner across all sources**
...
- [ ] **Passo 4: Update copy text in WarningBanner**
...
- [ ] **Passo 5: "Computing layout…" overlay**
...
- [ ] **Passo 2: Build dashboard so the script can import dist**

```bash
pnpm --filter @understand-anything/dashboard build
```

- [ ] **Passo 3: Run benchmark**

```bash
node understand-anything-plugin/packages/dashboard/scripts/benchmark-layout.mjs
```

Esperado: prints three lines. Verify Stage 1 < 200ms at 500 nodes, < 500ms at 3000 nodes per spec §8.3.
