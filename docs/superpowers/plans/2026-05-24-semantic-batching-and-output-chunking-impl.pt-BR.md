# Plano de Implementação de Batching Semântico e Output Chunking

> 🇬🇧 Versão original em inglês: [2026-05-24-semantic-batching-and-output-chunking-impl.md](./2026-05-24-semantic-batching-and-output-chunking-impl.md)

> **Para workers agênticos:** SUB-SKILL OBRIGATÓRIA: Use superpowers:subagent-driven-development (recomendado) ou superpowers:executing-plans para implementar este plano tarefa a tarefa. Os passos usam sintaxe de checkbox (`- [ ]`) para acompanhamento. **Todos os subagents despachados devem usar `model="opus"`** (convenção do projeto).

**Objetivo:** Substituir o batching baseado em contagem do file-analyzer por batching semântico Louvain (Fase 1.5), e adicionar output chunking defensivo no file-analyzer (60 nodes / 120 edges por parte), para que `/understand` pare de bater nos caps de saída do Bedrock OPUS e produza melhor cobertura de edges semânticos cross-batch. Um PR.

**Arquitetura:** Adicionar `compute-batches.mjs` (Fase 1.5) que executa Louvain sobre o grafo de imports de `scan-result.json` e escreve `batches.json` contendo `batchImportData` pré-construído + `neighborMap` (paths + símbolos exportados). O file-analyzer lê o neighborMap para emitir edges cross-batch com confiança, e divide sua própria saída em `batch-<i>-part-<k>.json` quando acima dos limites. O glob de `merge-batch-graphs.py` já aceita o naming multi-parte (sem mudança de código, apenas report no stderr + aviso de parte faltante).

**Stack:** Node.js ≥22 + pnpm ≥10, `graphology` + `graphology-communities-louvain` (novas deps), `@understand-anything/core` TreeSitterPlugin (existente), Vitest para testes `.mjs`, Python `unittest` para testes de `merge-batch-graphs.py`.

**Spec de origem:** [`docs/superpowers/specs/2026-05-24-semantic-batching-and-output-chunking-design.md`](../specs/2026-05-24-semantic-batching-and-output-chunking-design.md)

**Branch:** `feat/semantic-batching-and-output-chunking` (já criada).
...
## Tarefa 1: Adicionar dependências graphology

**Arquivos:**
- Modificar: `understand-anything-plugin/package.json`

- [ ] **Passo 1: Add deps to package.json**
...
Execute a partir de repo root:
```bash
pnpm install
```
Esperado: lockfile updates with graphology + graphology-communities-louvain; no other version churn.

- [ ] **Passo 3: Smoke test the imports work**

Execute a partir de `understand-anything-plugin/`:
```bash
...
```
Esperado: prints `graphology ok, edges: 1` and `louvain ok: {...}` with community ids assigned.
...
## Tarefa 2: Protótipo compute-batches.mjs (load + Louvain print)
...
**Arquivos:**
- Criar: `understand-anything-plugin/skills/understand/compute-batches.mjs`
- Criar: `understand-anything-plugin/skills/understand/test/fixtures/scan-result-3-cliques.json`

- [ ] **Passo 1: Create test fixture (3 disjoint import cliques)**
...
- [ ] **Passo 2: Write skeleton compute-batches.mjs (Louvain only, no neighborMap, no exports, no fallback)**
...
- [ ] **Passo 3: Run skeleton against the fixture**
...
- [ ] **Passo 4: (Optional) Run against this repo's scan-result.json if it exists**
...
## Tarefa 3: Escrever harness Vitest + primeiro unit test Louvain

**Arquivos:**
- Criar: `understand-anything-plugin/skills/understand/test_compute_batches.test.mjs`

- [ ] **Passo 1: Write failing test (Louvain produces 3 batches for 3 cliques)**
...
- [ ] **Passo 2: Run test, expect FAIL**

```bash
pnpm --filter @understand-anything/skill exec vitest run skills/understand/test_compute_batches.test.mjs -t "Louvain basic"
```

Esperado: FAIL — `compute-batches.mjs` skeleton from Task 2 only prints to stderr, doesn't write `batches.json`. Test fails on `readBatches` → ENOENT.

- [ ] **Passo 3: Make skeleton write batches.json**
...
## Tarefa 4: Enforcement de tamanho — dividir comunidades grandes demais
...
**Arquivos:**
- Modificar: `understand-anything-plugin/skills/understand/compute-batches.mjs`
- Modificar: `understand-anything-plugin/skills/understand/test_compute_batches.test.mjs`
- Criar: `understand-anything-plugin/skills/understand/test/fixtures/scan-result-large-community.json`

- [ ] **Passo 1: Create large-community fixture (40-node complete graph in one community)**
...
- [ ] **Passo 2: Write failing test (large community splits to ≤ 35)**
...
- [ ] **Passo 3: Run test, expect FAIL**

```bash
pnpm --filter @understand-anything/skill exec vitest run skills/understand/test_compute_batches.test.mjs -t "size enforcement"
```

Esperado: FAIL — at least one batch has 40 files; no warning emitted.

- [ ] **Passo 4: Implement WCC-style split + warning**
...
- [ ] **Passo 5: Run test, expect PASS**

```bash
pnpm --filter @understand-anything/skill exec vitest run skills/understand/test_compute_batches.test.mjs -t "size enforcement"
```

Esperado: PASS — 40 files split into 2 batches of 20 each, warning emitted.

- [ ] **Passo 6: Run prior test too, expect still PASS**
...
## Tarefa 5: Extração de exports via TreeSitterPlugin

**Arquivos:**
- Modificar: `understand-anything-plugin/skills/understand/compute-batches.mjs`
- Modificar: `understand-anything-plugin/skills/understand/test_compute_batches.test.mjs`

- [ ] **Passo 1: Write failing test (exports populated on real TS files)**
...
Esperado: FAIL — `batches.exportsByPath` is undefined.

- [ ] **Passo 3: Add TreeSitterPlugin loader + exports loop**
...
- [ ] **Passo 5: Run all tests, expect still PASS**
...
## Tarefa 6: Batching de não-código (Grupos A-E)

**Arquivos:**
- Modificar: `understand-anything-plugin/skills/understand/compute-batches.mjs`
- Modificar: `understand-anything-plugin/skills/understand/test_compute_batches.test.mjs`
- Criar: `understand-anything-plugin/skills/understand/test/fixtures/scan-result-non-code.json`

- [ ] **Passo 1: Create non-code fixture**
...
- [ ] **Passo 2: Write failing tests for each non-code group**
...
Esperado: FAIL on all four (non-code files currently end up nowhere — they're not in `codeFiles`, not in any batch).

- [ ] **Passo 4: Implement non-code grouping**
...
## Tarefa 7: batchImportData + neighborMap

**Arquivos:**
- Modificar: `understand-anything-plugin/skills/understand/compute-batches.mjs`
- Modificar: `understand-anything-plugin/skills/understand/test_compute_batches.test.mjs`

- [ ] **Passo 1: Write failing tests (batchImportData populated, neighborMap correct, excludes same-batch)**
...
- [ ] **Passo 2: Run tests, expect FAIL**

```bash
pnpm --filter @understand-anything/skill exec vitest run skills/understand/test_compute_batches.test.mjs -t "neighborMap"
```

Esperado: FAIL — `batchImportData` and `neighborMap` are currently empty `{}` on every batch.

- [ ] **Passo 3: Implement batchImportData + neighborMap construction**
...
- [ ] **Passo 5: Add neighborMap truncation test**
...
## Tarefa 8: Caminho de fallback + warning do Louvain

**Arquivos:**
- Modificar: `understand-anything-plugin/skills/understand/compute-batches.mjs`
- Modificar: `understand-anything-plugin/skills/understand/test_compute_batches.test.mjs`

- [ ] **Passo 1: Write failing test (Louvain crash → fallback, warning emitted, batches still valid)**
...
Esperado: FAIL — no fallback path exists; script crashes or produces `algorithm: "louvain"`.

- [ ] **Passo 3: Implement fallback**
...
Esperado: all PASS including new fallback test.
...
## Tarefa 9: Modo --changed-files

**Arquivos:**
- Modificar: `understand-anything-plugin/skills/understand/compute-batches.mjs`
- Modificar: `understand-anything-plugin/skills/understand/test_compute_batches.test.mjs`

- [ ] **Passo 1: Write failing test**
...
Esperado: FAIL — flag is unrecognized; output contains all batches.

- [ ] **Passo 3: Implement --changed-files filtering**
...
## Tarefa 10: file-analyzer.md — adicionar seção Cross-batch context (neighborMap)

**Arquivos:**
- Modificar: `understand-anything-plugin/agents/file-analyzer.md`

- [ ] **Passo 1: Insert the new section**
...
- [ ] **Passo 2: Verify the section was inserted correctly**
...
Esperado: all three lines exist, and the Cross-batch context line number is between Step 1's and Step 2's line numbers.
...
## Tarefa 11: file-analyzer.md — substituir Writing Results pelo protocolo multi-parte

**Arquivos:**
- Modificar: `understand-anything-plugin/agents/file-analyzer.md`

- [ ] **Passo 1: Replace the Writing Results section**
...
Esperado: first three exist, last `grep` returns non-zero (i.e. no match).
...
## Tarefa 12: SKILL.md — Fase 1.5 + reescrita da Fase 2 + reescrita do caminho Incremental

**Arquivos:**
- Modificar: `understand-anything-plugin/skills/understand/SKILL.md`

- [ ] **Passo 1: Insert Phase 1.5 after Phase 1**
...
- [ ] **Passo 2: Replace Phase 2 ANALYZE Full analysis path**
...
- [ ] **Passo 3: Replace Incremental update path**
...
Esperado: first three exist (compute-batches.mjs should appear at least 3 times — Phase 1.5 + Incremental); both check commands print "OK: ... absent".
...
## Tarefa 13: merge-batch-graphs.py — report multi-parte no stderr + aviso de parte faltante

**Arquivos:**
- Modificar: `understand-anything-plugin/skills/understand/merge-batch-graphs.py`

- [ ] **Passo 1: Replace the "Found N batch files:" report**
...
- [ ] **Passo 2: Verify the file still parses**
...
- [ ] **Passo 3: Smoke-test the existing test suite still passes**

```bash
cd understand-anything-plugin/skills/understand && python3 -m unittest test_merge_batch_graphs.py -v 2>&1 | tail -20
```

Esperado: all existing tests pass (we haven't broken anything).
...
## Tarefa 14: merge-batch-graphs.py — unit tests multi-parte

**Arquivos:**
- Modificar: `understand-anything-plugin/skills/understand/test_merge_batch_graphs.py`

- [ ] **Passo 1: Append TestMultiPart class**
...
## Tarefa 15: Gate de aceitação de integração (manual)

This task is a **gated manual checklist** — execute interactively, mark each item, do not auto-merge without all green.

**Arquivos:** none (this is a verification step)

- [ ] **Passo 1: Install + build clean**
...
Esperado: all succeed.

- [ ] **Passo 2: Sync local plugin into Claude Code's plugin cache for testing**
...
- [ ] **Passo 3: Start a fresh Claude Code session and run /understand --full on this repo**
...
- [ ] **Passo 4: Sanity-check batches.json contents**
...
- [ ] **Passo 5: Run on a small repo (5-10 files) to verify fallback batch path**
...
- [ ] **Passo 6: Run on a ~100-file repo to validate the bug fix**
...
## Tarefa 16: Bump de versão nos 5 arquivos

Per project CLAUDE.md: when pushing to remote, bump version in **all five** files listed.

**Arquivos:**
- Modificar: `understand-anything-plugin/package.json`
- Modificar: `understand-anything-plugin/.claude-plugin/plugin.json`
- Modificar: `.claude-plugin/plugin.json`
- Modificar: `.cursor-plugin/plugin.json`
- Modificar: `.copilot-plugin/plugin.json`

- [ ] **Passo 1: Determine new version**
...
- [ ] **Passo 2: Confirm all five files have the same current version**
...
Esperado: all five print `"version": "2.7.4"` (or whatever the current version is — use that as the baseline). If they diverge, stop and reconcile with the user.

- [ ] **Passo 3: Bump each file from `2.7.4` to `2.8.0`**

Use the Edit tool on each of the five files. For each, replace `"version": "2.7.4"` with `"version": "2.8.0"`.

- [ ] **Passo 4: Verify all five updated**
...
Esperado: all five print `"version": "2.8.0"`.
...
- [ ] **Passo 6: Push branch and open PR**
